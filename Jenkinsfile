...
pipeline {
  ...
  openshift.withProject("jenkins-cicd") {
    ...
    if(!buildConfigExists){ 
      openshift.newBuild("--cicd", "--docker-image=registry.redhat.io/jboss-eap-7/eap74-openjdk8-openshift-rhel7", "--binary") 
    } 
    
    openshift.selector("bc", "cicd").startBuild("--from-file=target/sample.war", "--follow") } }
